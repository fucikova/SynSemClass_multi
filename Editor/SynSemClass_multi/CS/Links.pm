...
package SynSemClass_multi::CS::Links;
...
sub create_widget {
  ...
  my $czechwn_links = SynSemClass_multi::CS::LexLink->new($data, undef, $czechwn_frame, "WordNet (czech)",
													qw/-height 3/);
  $czechwn_links->configure_links_widget("czechwn");

  my $vallex_frame=$w->Frame(-takefocus=>0);
  $vallex_frame->pack(qw/-fill x -padx 4/);
  my $vallex_links = SynSemClass_multi::CS::LexLink->new($data, undef, $vallex_frame, "Vallex",
													qw/-height 3/);
  ...
  my $czengvallex_links = SynSemClass_multi::CS::LexLink->new($data, undef, $czengvallex_frame, "CzEngVallex",
													qw/-height 3/);
  ...
  my $pdtvallex_links = SynSemClass_multi::CS::LexLink->new($data, undef, $pdtvallex_frame, "PDT-Vallex",
													qw/-height 3/);
...
# LexLink widget
#
package SynSemClass_multi::CS::LexLink;
use base qw(SynSemClass_multi::FramedWidget);
...
sub fetch_czengvallex_cs_links{
  ...
  my $lang = $self->data()->getClassMemberAttribute($classmember, 'lang');
  my $text="";
...
sub getNewLink{
  ...
  while ($ok){
	if ($link_type eq "pdtvallex"){
	...
	}elsif($link_type eq "wn" or $link_type eq "czechwn"){
		if ($new_value[0] eq ""){
		...
		my ($idpref, $sense)=SynSemClass_multi::Sort::parse_vallex_id($new_value[0]);
		...
		if (!$vallex4_0_mapping->{id}->{$new_value[0]}->{validid}){
			...
			if (scalar @val_ids > 1){
				...
				if ($answer eq "Yes"){
					($idpref, $sense)=SynSemClass_multi::Sort::parse_vallex_id($val_ids[0]);
...
sub show_vallex_editor_dialog{
  my ($self, $d, $action,$focused,@value)=@_;
    ...
    my $idpref_s="";
	my $sense_s="";
	my ($idpref_s, $sense_s)=SynSemClass_multi::Sort::parse_vallex_id($id_s);
