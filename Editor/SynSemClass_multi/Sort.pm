...
SynSemClass_multi::Sort

=cut

package SynSemClass_multi::Sort;
require SynSemClass_multi::CS::Sort;
require SynSemClass_multi::EN::Sort;
require SynSemClass_multi::DE::Sort;
...
sub sort_links($$){
	...
	my $pack = "SynSemClass_multi::" . uc($lang) . "::Sort";
...
sub equal_lemmas{
	...
	foreach ($l1, $l2){
		$_=~s/([áéěíóúůýžščřďťňÁÉĚÍÓÚŮÝŽŠČŘĎŤŇäöüëËÄÖÜ])/_\1/g;
		$_=~tr/[áéěíóúůýžščřďťňÁÉĚÍÓÚŮÝŽŠČŘĎŤŇäëüöÄËÜÖ]/[aeeiouuyzscrdtnAEEIOUUYZSCRDTNaeuoAEUO]/;
		$_=~s/ß/_z/g;
		$_=~s/SS/_Z/g;
	}
	if ($l1 eq $l2){
