...
package SynSemClass_multi::DE::Links;
...
sub create_widget {
  ...
  my $framenetDe_links = SynSemClass_multi::DE::LexLink->new($data, undef, $framenetDe_frame, "FrameNet Des Deutschen",
													qw/-height 3/);
  $framenetDe_links->configure_links_widget("fnd");
  
  my $woxikon_frame=$w->Frame(-takefocus=>0);
  $woxikon_frame->pack(qw/-fill x -padx 4/);
  my $woxikon_links = SynSemClass_multi::DE::LexLink->new($data, undef, $woxikon_frame, "Woxikon",
													qw/-height 3/);
  ...
  my $paracrawlGe_links = SynSemClass_multi::DE::LexLink->new($data, undef, $paracrawlGe_frame, "ParaCrawl German",
													qw/-height 3/);
  $paracrawlGe_links->configure_links_widget("paracrawl_ge");
  
  my $gup_frame=$w->Frame(-takefocus=>0);
  $gup_frame->pack(qw/-fill x -padx 4/);
  my $gup_links = SynSemClass_multi::DE::LexLink->new($data, undef, $gup_frame, "GUP",
													qw/-height 3/);
  $gup_links->configure_links_widget("gup");

  my $valbu_frame=$w->Frame(-takefocus=>0);
  $valbu_frame->pack(qw/-fill x -padx 4/);
  my $valbu_links = SynSemClass_multi::DE::LexLink->new($data, undef, $valbu_frame, "VALBU",
													qw/-height 3/);
...
sub get_frame_elements{
  ...
  if ($lexidref eq 'gup'){
	my $id = $idref;
	$id =~ s/-/./;
	push @elements, @{$SynSemClass_multi::DE::LexLink::gup_mapping->{roleset_id}->{$id}->{args}} if (defined $SynSemClass_multi::LexLink_De::gup_mapping->{roleset_id}->{$id}->{args});

  }elsif ($lexidref eq 'valbu'){
  	my ($id, $sense)=split('-', $idref, 2);
	
	if (defined $SynSemClass_multi::DE::LexLink::valbu_mapping->{args}->{$id}->{$sense}){
		foreach (@{$SynSemClass_multi::DE::LexLink::valbu_mapping->{args}->{$id}->{$sense}}){ 
			$_=~ s/_XOR/ XOR/;
			$_=~ s/_OR/ OR/;
...
# LexLink widget
#
package SynSemClass_multi::DE::LexLink;
use base qw(SynSemClass_multi::FramedWidget);
